...
Что нужно сделать:
В последнем видео мы писали программу, вычисляющую сумму налога по
прогрессивной шкале в зависимости от полученного заработка:
...
вычисляется более сложным способом: налоговая ставка 30% на доход
выше 50 000 означает, что 30% уплачивается не со всей суммы,
...
void practicalWork9() {
	...
	if (profit >= 50000) {
		int tax = ((profit - 50000) * 30 / 100) + (40000 * 20 / 100) + (10000 * 13 / 100);
		std::cout << "Размер налога (30%) равен: " << tax << "\n";
	}
	else if (profit >= 10000) {
		int tax = ((profit - 10000) * 30 / 100) + (10000 * 13 / 100);
